...
void LoadEditorFont(ImGuiIO& io)
{
    ...
    {
        // Use the system font without redistributing it. Read through a native path,
        // not the C runtime's locale-dependent narrow filename overload.
        for (const wchar_t* name : {L"msyh.ttc", L"simsun.ttc"})
        {
            const auto path = std::filesystem::path(windowsDirectory) / L"Fonts" / name;
            std::ifstream stream(path, std::ios::binary | std::ios::ate);
            if (!stream) continue;
            const auto size = stream.tellg();
            if (size <= 0 || size > 64 * 1024 * 1024) continue;
            void* bytes = IM_ALLOC(static_cast<size_t>(size));
            if (!bytes) continue;
            stream.seekg(0);
            if (!stream.read(static_cast<char*>(bytes), static_cast<std::streamsize>(size)))
            {
                IM_FREE(bytes);
                continue;
            }
            // ImGui owns the allocation. Its 1.92 renderer lazily loads required glyphs.
            io.Fonts->AddFontFromMemoryTTF(bytes, static_cast<int>(size), 16.0f);
            return;
        }
    }
#endif
    io.Fonts->AddFontDefault();
    std::cerr << "Chinese editor font unavailable; Unicode file loading is still supported.\n";
}
}
...
void EditorLayer::initialize(GLFWwindow* window)
{
    ...
    dialogDockId = 0;
    inputCapture = {};
    viewport.refreshDebugLabels(resources);
}
