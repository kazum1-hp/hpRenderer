...
void IBLPrecompute::initialize(AssetManager& resources)
{
    skyboxShader = resources.GetShader(ShaderId::EnvironmentCapture);
    irradianceShader = resources.GetShader(ShaderId::Irradiance);
    prefilterShader = resources.GetShader(ShaderId::Prefilter);
    brdfShader = resources.GetShader(ShaderId::Brdf);
    cube = Rendering::CreateCube();
    screenQuad = Rendering::CreateScreenQuad();
}
...
std::unique_ptr<EnvironmentGpuResources> IBLPrecompute::bake(GLuint hdrTexture)
{
    ...
    std::array<glm::mat4, 6> projViewMatrix = CalculateCubemapMatrices(glm::vec3(0.0f));
    skyboxShader->use();
