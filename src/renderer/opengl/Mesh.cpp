	...
	:textures(texs)
{
	...
	{
		throw std::logic_error("Cannot create Mesh: OpenGL 3.3 functions are unavailable.");
	}
	...
	if (err != GL_NO_ERROR)
	{
		cleanUp(); // A failed constructor will not invoke the destructor.
		throw std::runtime_error("GL error after mesh setup: " + std::to_string(err));
	}
}
